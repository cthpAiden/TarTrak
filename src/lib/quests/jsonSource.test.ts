    ...
    data: {
      maps: {
        m1: {
          ...
          extracts: [{ id: "e1", name: "Cellars", faction: "pmc", position: { x: 1, y: 2, z: 3 } }],
          transits: [{ id: "12", description: "FAC_TRANSIT_12_DESC", map: "m2", position: { x: 4, y: 5, z: 6 } }],
          spawns: [{ position: { x: 7, y: 8, z: 9 }, sides: ["all"], categories: ["player"], zoneName: "z1" }],
          lootContainers: [{ lootContainer: "lc1", position: { x: 10, y: 11, z: 12 } }],
          lootLoose: [{ position: { x: 13, y: 14, z: 15 }, items: ["i1", "i2"] }],
          locks: [
          ...
          switches: [{ id: "sw1", name: "lever", position: { x: 25, y: 26, z: 27 } }],
...
describe("JSON_FILES", () => {
  it("names the six source files", () => {
    expect(JSON_FILES).toEqual(["maps", "maps_en", "tasks", "tasks_en", "traders", "traders_en"]);
...
describe("toQuestData", () => {
  ...
  it("maps every layer of a map", () => {
    ...
    expect(m.extracts).toEqual([{ id: "e1", name: "Cellars", faction: "pmc", position: { x: 1, y: 2, z: 3 } }]);
    expect(m.transits).toEqual([{ id: "12", description: "Transit to Woods", position: { x: 4, y: 5, z: 6 } }]);
    expect(m.spawns).toEqual([{ zoneName: "z1", position: { x: 7, y: 8, z: 9 }, sides: ["all"], categories: ["player"] }]);
    expect(m.lootContainers).toEqual([
      { lootContainer: { id: "lc1", name: "Weapon box", normalizedName: "weapon-box" }, position: { x: 10, y: 11, z: 12 } },
    ]);
    expect(m.lootLoose).toEqual([{ position: { x: 13, y: 14, z: 15 }, items: ["i1", "i2"] }]);
    expect(m.locks).toEqual([
      { lockType: "door", key: "k1", position: { x: 16, y: 17, z: 18 } },
    ...
    expect(m.switches).toEqual([{ id: "sw1", name: "lever", position: { x: 25, y: 26, z: 27 } }]);
